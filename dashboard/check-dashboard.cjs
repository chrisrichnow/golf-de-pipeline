...
(async () => {
  ...
  try {
    ...
    const summaries = data.summaries.filter(x => x.season_year === '2026');
    const players = data.players.filter(x => x.season_year === '2026');
    const teams = data.teams.filter(x => x.season_year === '2026');
